// ZKG Counter Upload – client (fully fixed version)
const CONFIG = {
...
async function guessNamesFromPortraits(imgUrl, hashDb) {
  ...
  const namesA = [], namesD = [];
  for (const b of attackBoxes) namesA.push(bestMatch(await cropAndHash(img, ...b), hashDb));
  for (const b of defenseBoxes) namesD.push(bestMatch(await cropAndHash(img, ...b), hashDb));
